...
public class STShortWU implements STClass{
	...
	private STShortWU(short a_short){
	...
	public void testEquals(){
		Query q = st.query();
		q.constrain(new STShortWU((short)0));  
	...
	public void testGreater(){
		Query q = st.query();
		Constraint c = q.constrain(new STShortWU((short)9));
	...
	public void testSmaller(){
		Query q = st.query();
		Constraint c = q.constrain(new STShortWU((short)1));
	...
	public void testNotContains(){
		Query q = st.query();
		Constraint c = q.constrain(new STShortWU((short)0));
	...
	public void testLike(){
		Query q = st.query();
		Constraint c = q.constrain(new STShortWU((short)90));
		q.descend(DESCENDANT).constraints().like();
		st.expectOne(q, store()[3]);
		q = st.query();
		c = q.constrain(new STShortWU((short)10));
		q.descend(DESCENDANT).constraints().like();
		st.expectNone(q);
	}
	...
	public void testIdentity(){
		...
		STShortWU identityConstraint = (STShortWU)set.next();
	...
	public void testConstraints(){
		Query q = st.query();
		q.constrain(new STShortWU((short)1));
		q.constrain(new STShortWU((short)0));
	...
	public void testEvaluation(){
		Query q = st.query();
		q.constrain(new STShortWU());
		q.constrain(new Evaluation() {
			public void evaluate(Candidate candidate) {
				STShortWU sts = (STShortWU)candidate.getObject();
